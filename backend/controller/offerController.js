...
export const validateOffer = async (req, res) => {
  try {
    ...
    if (!offer)
      return res
        .status(404)
        .json({ success: false, message: "Offer code not found or inactive" });
    if (offer.expiryDate < new Date()) {
      return res
        .status(400)
        .json({ success: false, message: "Offer code expired" });
    }
    if (orderAmount < offer.minOrderAmount) {
      return res.status(400).json({
        success: false,
        message: `Minimum order amount for this offer is ₹${offer.minOrderAmount}`,
      });
    }
    if (offer.forNewUsersOnly) {
      if (!userId)
        return res
          .status(400)
          .json({ success: false, message: "User ID required for this offer" });
      ...
      if (Array.isArray(user.orders) && user.orders.length > 0) {
        return res
          .status(400)
          .json({ success: false, message: "Offer valid for new users only" });
      }
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Server error while validating offer" });
  }
};
